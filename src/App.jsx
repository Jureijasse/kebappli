...
export default function KebApp() {
  // Charger depuis localStorage ou fallback initial
  ...
  // Sauvegarder localStorage à chaque changement
  useEffect(() => {
  ...
  const handleLogin = () => {
    ...
    // Charger les avis user depuis compte
  ...
  // Handle login or register depending on mode
  ...
  // Marker click -> ouvrir formulaire avis
  ...
  const handleSubmitReview = (e) => {
    ...
    if (!text || !note) {
      alert("Merci de remplir texte et note");
      return;
    }
    ...
    if (editingReview) {
      updatedReviews = userReviews.map((r) =>
        r.kebabId === selectedKebab.id ? { ...r, text, note } : r
      );
    } else {
      updatedReviews = [...userReviews, { kebabId: selectedKebab.id, text, note }];
    }

    setUserReviews(updatedReviews);

    // Met à jour le compte connecté avec les avis
    setAccounts((accs) =>
      accs.map((acc) =>
  ...
  const sendFriendRequest = () => {
    ...
    if (friendId === auth.id.toLowerCase()) {
      alert("Vous ne pouvez pas vous ajouter vous-même");
      return;
    }
    if (friends.some((f) => f.id.toLowerCase() === friendId)) {
    ...
    // On simule que l’ami accepte automatiquement (pas de backend)
    ...
    <div className="h-screen w-screen bg-orange-100 flex flex-col font-sans">
                ...
                <Popup>
                  <div>
                    ...
                    <Button
                      ...
                      onClick={() => handleMarkerClick(kebab)}
                    >
                      Rédiger un avis
                    </Button>
                    <div className="mt-2 text-sm max-h-32 overflow-auto">
                      {friends
                        .flatMap((u) =>
                          u.reviews
                            .filter((r) => r.kebabId === kebab.id)
                            .map((r) => `${u.name} : ${r.text} (${r.note}/5)`)
                        )
                        .map((txt, i) => (
                          <div key={i}>{txt}</div>
                        ))}
                      {userReviews
                        .filter((r) => r.kebabId === kebab.id)
                        .map((r, i) => (
                          <div key={`user-${i}`} className="font-semibold text-orange-700">
                            Moi : {r.text} ({r.note}/5)
                          </div>
                        ))}
                    </div>
      ...
      {auth.loggedIn && view === "review" && selectedKebab && (
        <form onSubmit={handleSubmitReview} className="flex-1 p-4 bg-orange-100 overflow-auto">
          ...
          <textarea
            name="description"
            placeholder="Rédige ton avis ici..."
            className="w-full h-24 p-2 border rounded mb-2"
            defaultValue={editingReview ? editingReview.text : ""}
            required
          />
          <div className="flex items-center mb-2">
            ...
            <select
              ...
              defaultValue={editingReview ? editingReview.note : 5}
              required
            >
              {[1, 2, 3, 4, 5].map((n) => (
            ...
            {friends.map((f) => (
              <li
                ...
                onClick={() =>
                  alert(
                    f.reviews.length > 0
                      ? f.reviews.map((r) => `${r.text} (${r.note}/5)`).join("\n")
